ALTER TABLE public.project_availability_step_settings
    ADD COLUMN IF NOT EXISTS minimum_notice_hours integer NOT NULL DEFAULT 24;

ALTER TABLE public.project_availability_step_settings
    DROP CONSTRAINT IF EXISTS project_availability_step_settings_minimum_notice_hours_check;

ALTER TABLE public.project_availability_step_settings
    ADD CONSTRAINT project_availability_step_settings_minimum_notice_hours_check
    CHECK (minimum_notice_hours >= 0 AND minimum_notice_hours <= 8760);

CREATE OR REPLACE FUNCTION public.trg_reject_past_project_availability_slot()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
    ...
    v_minimum_notice_hours integer := 24;
BEGIN
    IF NEW.is_active IS NOT TRUE THEN
        RETURN NEW;
    END IF;

    IF NEW.slot_date IS NULL OR NEW.start_time IS NULL THEN
        RAISE EXCEPTION 'Availability slot date and start time are required';
    END IF;

    SELECT GREATEST(COALESCE(st.minimum_notice_hours, 24), 0)
      INTO v_minimum_notice_hours
      FROM public.project_availability_step_settings st
     WHERE st.project_id = NEW.project_id
       AND st.field_id = NEW.field_id;

    v_minimum_notice_hours := GREATEST(COALESCE(v_minimum_notice_hours, 24), 0);
    v_start_at := public.project_availability_slot_starts_at(NEW.slot_date, NEW.start_time, NEW.timezone);
    IF v_start_at IS NULL OR v_start_at <= now() + make_interval(hours => v_minimum_notice_hours) THEN
        RAISE EXCEPTION 'Availability slot start time must be after the minimum booking notice window';
    END IF;
...
BEGIN
    ...
    WHERE s.project_id = p_project_id
      ...
      AND public.project_availability_slot_starts_at(s.slot_date, s.start_time, s.timezone)
          > now() + make_interval(hours => GREATEST(COALESCE(st.minimum_notice_hours, 24), 0))
...
BEGIN
    ...
    SELECT s.project_id, s.field_id, public.project_availability_slot_starts_at(s.slot_date, s.start_time, s.timezone)
      INTO v_slot_project_id, v_slot_field_id, v_slot_start_at
      FROM public.project_availability_slots s
    ...
    IF v_minimum_notice_hours IS NULL THEN
        RAISE EXCEPTION 'Booking is not enabled for this step';
    END IF;

    IF v_slot_start_at IS NULL OR v_slot_start_at <= now() + make_interval(hours => v_minimum_notice_hours) THEN
        RAISE EXCEPTION 'This slot is inside the minimum booking notice window. Please choose another slot.';
    END IF;
    ...
    FOR v_candidate IN
        WITH candidate_slots AS (
            ...
            WHERE s.id = ANY(v_candidate_ids)
              ...
              AND s.is_active IS TRUE
              AND public.project_availability_slot_starts_at(s.slot_date, s.start_time, s.timezone)
                  > now() + make_interval(hours => v_minimum_notice_hours)
        )
        SELECT
            cs.*,
            (
