CREATE OR REPLACE FUNCTION public.project_availability_slot_starts_at(
    p_slot_date date,
    p_start_time time,
    p_timezone text
)
...
STABLE
AS $$
    SELECT public.project_availability_slot_instant(
        p_slot_date,
        p_start_time,
...
BEGIN
    ...
    IF v_start_at IS NULL OR v_start_at <= now() THEN
        RAISE EXCEPTION 'Availability slot start time must be in the future';
    END IF;
...
DROP TRIGGER IF EXISTS trg_reject_past_project_availability_slot ON public.project_availability_slots;
CREATE TRIGGER trg_reject_past_project_availability_slot
BEFORE INSERT OR UPDATE OF is_active, slot_date, start_time, timezone
ON public.project_availability_slots
FOR EACH ROW
EXECUTE FUNCTION public.trg_reject_past_project_availability_slot();
...
BEGIN
    ...
    WHERE s.project_id = p_project_id
      ...
      AND public.project_availability_slot_starts_at(s.slot_date, s.start_time, s.timezone) > now()
...
BEGIN
    ...
    IF v_slot_start_at IS NULL OR v_slot_start_at <= now() THEN
        RAISE EXCEPTION 'This slot has already passed. Please choose another slot.';
    END IF;
