...
export function getSlotStartDate(slot) {
    return zonedDateTimeToDate(slot?.slot_date, slot?.start_time, slot?.timezone || "Asia/Bangkok");
}

export function isAvailabilitySlotInFuture(slot, referenceDate = new Date()) {
    const startDate = getSlotStartDate(slot);
    return startDate instanceof Date && !Number.isNaN(startDate.getTime()) && startDate.getTime() > referenceDate.getTime();
}

export function normalizeMinimumNoticeHours(value, fallback = 0) {
    const number = Number(value);
    if (!Number.isFinite(number)) return fallback;
    return Math.max(0, Math.min(8760, Math.floor(number)));
}

export function isAvailabilitySlotBookable(slot, minimumNoticeHours = 0, referenceDate = new Date()) {
    const startDate = getSlotStartDate(slot);
    if (!(startDate instanceof Date) || Number.isNaN(startDate.getTime())) return false;
    const noticeMs = normalizeMinimumNoticeHours(minimumNoticeHours) * 60 * 60 * 1000;
    return startDate.getTime() > referenceDate.getTime() + noticeMs;
}
...
export function getBookingSessionStatusLabel(status) {
...
export function formatBookingSessionStatus(booking) {
    if (!booking) return "";
    return getBookingSessionStatusLabel(booking.session_status);
}
