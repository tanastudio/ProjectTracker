...
export function attachTicketNavBadge({ supabase, navElement, getProjectId, getProjectIds, userId }) {
    ...
    async function refresh() {
        ...
        try {
            ...
            const displayCount = totals.unreadCount > 0 ? totals.unreadCount : totals.openCount;

            badge.textContent = displayCount > 99 ? "99+" : String(displayCount);
            badge.hidden = displayCount <= 0;
            badge.classList.toggle("is-open-only", totals.unreadCount <= 0 && totals.openCount > 0);
