...
function normalizeTicketCountRow(row) {
    return {
        unreadCount: Math.max(0, Number(row?.unread_count ?? row?.unreadCount ?? 0) || 0),
        openCount: Math.max(0, Number(row?.open_count ?? row?.openCount ?? 0) || 0),
    };
}
...
async function loadUnreadTicketCountsByProjectRpc({ supabase, projectIds }) {
    if (typeof supabase?.rpc !== "function") return null;

    const { data, error } = await supabase.rpc("get_ticket_counts_for_projects", {
    ...
    return new Map((data || []).map((row) => [
        ...
        normalizeTicketCountRow(row),
    ]).filter(([projectId]) => projectId));
}

async function loadUnreadTicketCountsByProjectFallback({ supabase, projectIds, userId }) {
...
export async function loadUnreadTicketCountsByProject({ supabase, projectIds, userId }) {
    ...
    try {
        const rpcCounts = await loadUnreadTicketCountsByProjectRpc({
            supabase,
            projectIds: uniqueProjectIds,
        });
        if (rpcCounts) return rpcCounts;
    } catch (error) {
        console.warn("ticket count RPC unavailable; using browser fallback:", error?.message || error);
    }

    return loadUnreadTicketCountsByProjectFallback({
        supabase,
        projectIds: uniqueProjectIds,
        userId,
    });
